...
gboolean getSignUpDataUsername() {
...
gboolean getSignUpDataPasswords() {
...
void onBtnSignUpMiniClicked(GtkButton *sign_up_mini_button) {
    ...
    if (!builder) {
        ...
        gtk_builder_add_from_file(builder, "resources/client/resources/reg_login.glade", NULL);
    }
...
void edit_messages(/*GtkMenuItem *menuitem, gpointer data*/)
{
...
void delete_messages(/*GtkMenuItem *menuitem, gpointer data*/)
{
    // code to delete the selected message
    g_print("Button 'Delete message'\n");
    /*gint result;*/
    if_deleted = true;
    update_in_model(/*selected_chat,*/ selected_message_id, message_buffer);
    gtk_tree_selection_unselect_all(messages_selection);
...
void onSignInClicked() {
    if (!builder_chat) { // Перевірка, чи конструктор вже існує
        ...
        gtk_builder_add_from_file(builder_chat, "client/resources/chat.glade", NULL); // Завантажити файл client/resources/chat.glade
    }
...
void onProfileButtonClicked(/*GtkButton *profile_button*/) {
    if (!builder_chat) { // Перевірка, чи конструктор вже існує
        builder_chat = gtk_builder_new(); // Створити конструктор
        gtk_builder_add_from_file(builder_chat, "client/resources/chat.glade", NULL); // Завантажити файл client/resources/chat.glade
    }
...
gboolean getChangeDataPasswords() {
...
void onChangeButtonOnPasswButtonClicked(/*GtkWidget *change_button_on_passw_change_window, gpointer data*/) {
    if (!builder_chat) { // Перевірка, чи конструктор вже існує
        builder_chat = gtk_builder_new(); // Створити конструктор
        gtk_builder_add_from_file(builder_chat, "client/resources/chat.glade", NULL); // Завантажити файл client/resources/chat.glade
    }
...
void onAddChatButtonClicked(/*GtkButton *add_chat_button*/) {
    if (!builder_chat) { // Перевірка, чи конструктор вже існує
        builder_chat = gtk_builder_new(); // Створити конструктор
        gtk_builder_add_from_file(builder_chat, "client/resources/chat.glade", NULL); // Завантажити файл client/resources/chat.glade
    }
...
void print_massages(/*GtkButton *send_message_button, gpointer data*/) {
    g_print("Сообщение: %s\n", message_buffer);
    ...
    if (message_buffer[0] != '\0') {
        ...
        GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file("client/resources/icon_massage.png", NULL);
...
void getMessage(/*GtkButton *send_message_button, gpointer data */) {
...
gboolean on_enter_press(/*GtkWidget *widget, */GdkEventKey *event, gpointer user_data) {
...
void day_theme(__attribute__((unused)) GtkWidget *button, gboolean state/*, void *data*/) {
    ...
    if (state) {
        ...
        gtk_css_provider_load_from_path(provider, "client/resources/style1.css", NULL);
    } else {
        // Состояние выключено: загрузить ночную тему
        gtk_css_provider_load_from_path(provider, "client/resources/style.css", NULL);
    }
...
void onCreateChatButtonClicked(/*GtkButton *b, gpointer data*/)
{
    gtk_entry_set_text(GTK_ENTRY(search_entry_username), "");
...
int main(int argc, char **argv) {
    ...
    if (fcntl(client->client_socket, F_SETFL, flags | O_NONBLOCK) == -1) {
    ...
    builder = gtk_builder_new();
    gtk_builder_add_from_file(builder, "client/resources/reg_login.glade", NULL);
    ...
    gtk_builder_add_from_file(builder_chat, "client/resources/chat.glade", NULL);
    ...
    gtk_css_provider_load_from_file(provider, g_file_new_for_path("client/resources/style.css"), NULL); // Завантажити CSS з файлу
    create_message_side();

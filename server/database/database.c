...
sqlite3 *db_file_open(void) {
    ...
    if (sqlite3_open(DATABASE_FILE, &db) != SQLITE_OK) {
        char msg[200];
        sprintf(msg, "DB: Could not open the database: [%s]", sqlite3_errmsg(db));
        log_to_file(msg);
        sqlite3_close(db);
        return NULL;
    }
...
bool db_create(void) {
    ...
    if (!sql_file) {
        char msg[200];
        sprintf(msg, "DB: Could not create the database tables: [%d | %s]", sqlite3_errcode(db), sqlite3_errmsg(db));
        log_to_file(msg);
        sqlite3_close(db);
        return false;
    }
    ...
    if (fgets(buffer, BUF_SIZE, sql_file) == NULL) {
        char msg[200];
        sprintf(msg, "DB: Could not read the file with SQL for creating database tables: [%d | %s]", sqlite3_errcode(db), sqlite3_errmsg(db));
        log_to_file(msg);
//        mx_strdel(&buffer); // todo change to standart foo?
    ...
    if (sqlite3_exec(db, buffer, NULL, 0, &err_message) != SQLITE_OK) {
        char msg[500];
        sprintf(msg, "DB: Could not execute the SQL for creating database tables: [%d | %s | %s]", sqlite3_errcode(db), sqlite3_errmsg(db), err_message);
        log_to_file(msg);
//        log_to_file("DB: Could not execute the SQL for creating database tables: [%d | %s | %s]", sqlite3_errcode(db), sqlite3_errmsg(db), err_message); //todo check err_message
    ...
    sqlite3_close(db);
    log_to_file("DB: Database successfully created");
    return true;
}
...
t_user_data *db_get_user_data(sqlite3 *db, char *username) {
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &sql_statement, NULL) != SQLITE_OK) {
        char msg[200];
        sprintf(msg, "DB: Could not create the prepared statement object: [%d | %s]", sqlite3_errcode(db), sqlite3_errmsg(db));
        log_to_file(msg);
    ...
    if (sqlite3_bind_text(sql_statement, 1, username, -1, NULL) != SQLITE_OK) { // Прив'язує рядок тексту до параметра "?" у запиті "WHERE users.username = ?"
        char msg[200];
        sprintf(msg, "DB: Could not bind value to the prepared statement object: [%d | %s]", sqlite3_errcode(db), sqlite3_errmsg(db));
        log_to_file(msg);
        sqlite3_finalize(sql_statement);
        return NULL;
    }
    ...
    if (sqlite3_step(sql_statement) == SQLITE_ROW) { // Якщо запит успішно виконався і повернув рядок даних
        ...
        if (sqlite3_column_count(sql_statement) != USERS_TABLE_COLUMNS_COUNT) {
            char msg[200];
            sprintf(msg, "DB: The prepared operator returns an invalid number of columns in the resulting set: [%d | %s]", sqlite3_errcode(db), sqlite3_errmsg(db));
            log_to_file(msg);
            sqlite3_finalize(sql_statement);
            return NULL;
        }
        ...
        user_data->username = strdup((const char *) sqlite3_column_text(sql_statement, 1)); // or mx_strdup()
        user_data->password = (char *) strdup((const char *)sqlite3_column_text(sql_statement, 2)); // or mx_strdup()
        user_data->icon_id = sqlite3_column_int(sql_statement, 3);
    } else {
        char msg[200];
        sprintf(msg, "DB: Could not execute the SQL statement: [%d | %s]", sqlite3_errcode(db), sqlite3_errmsg(db));
        log_to_file(msg);
        sqlite3_finalize(sql_statement);
        return NULL;
    }

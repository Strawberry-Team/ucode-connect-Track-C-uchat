...
void send_status_response(SSL *ssl, t_request_type request_type, t_status_type status) {
    ...
    if (!json) {
        log_to_file("Could not create the cJSON object to send a status response to the client", CJSON_ERROR);
        cJSON_Delete(json);
        return;
    }
    ...
    if (!cJSON_AddNumberToObject(json, "request_type", request_type)
        ...
        log_to_file("Could not add the data in the cJSON object to send a status response to the client", CJSON_ERROR);
        cJSON_Delete(json);
        return;
    }
    ...
    if (bytes_written <= 0) {
        log_to_file("Could not write JSON string over the TLS/SSL connection to send a login response to the client", CJSON_ERROR);
    }
...
void send_auth_response(SSL *ssl, t_request_type request_type, t_status_type status, t_user_data *user_data) {
    ...
    if (!json
        || !json_data_obj) {
        log_to_file("Could not create the cJSON object to send an authentication response to the client", CJSON_ERROR);
    ...
    if (!cJSON_AddItemToObject(json, "data", json_data_obj)
        ...
        log_to_file("Could not add the data in the cJSON object to send an authentication response to the client", CJSON_ERROR);
        send_status_response(ssl, request_type, ERROR_JSON);
        cJSON_Delete(json);
        return;
    }
    ...
    if (bytes_written <= 0) {
        log_to_file("Could not write JSON string over the TLS/SSL connection to send an authentication response to the client", CJSON_ERROR);
        free(json_string);
    }
